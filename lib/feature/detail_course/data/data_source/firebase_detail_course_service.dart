...
class FirebaseDetailCourseService implements DetailDataSource {
  ...
  @override
  Future<Result<List<SectionModel>>> getLessons(String id) async {
    DocumentReference<Map<String, dynamic>> documentReference =
    ...
    try {
      DocumentSnapshot<Map<String, dynamic>> result =
          await documentReference.get();
      if (result.exists) {
        return Result.success(List<SectionModel>.from(
            result["lessons"]!.map((item) => SectionModel.fromJson(item))));
      } else {
